...
se conservan en \texttt{data/gold/run\_manifest.json}.
...
\node[barra] (tab) at (6.9,-9.65) {Tableau · CSV reproducibles + dashboard\\ {\scriptsize\color{muted}cinco conjuntos ordenados de forma determinista}};
...
\texttt{unittest} & Verificación automática de parsing, identidad, contratos, Gold, DuckDB y export Tableau\\
...
\subsection{Procedencia desde el primer byte}
...
La evaluación se apoya en un marco de pruebas reproducible con perfiles de
carga de distinto tamaño y métricas conservadas en JSON. Ninguna
cifra de rendimiento se publica en la memoria sin su ejecución registrada.
...
La batería de pruebas se ejecuta con \texttt{unittest}. Cubre el parsing, la
procedencia, la identidad, los contratos entre capas, el estado vigente, el
enriquecimiento, las vistas DuckDB y el export a Tableau. Las suites de Spark
se ejecutan cuando está instalado el extra correspondiente; la validación
offline no necesita acceso a las fuentes oficiales.

Las pruebas incluyen una comprobación de colisiones: ante dos contenidos
distintos bajo el mismo identificador la
transformación debe fallar, nunca elegir. La clasificación tecnológica se
conserva como baseline histórico, pero no forma parte de la cadena E2E final
ni se presenta como evaluación principal porque no existe una muestra humana
anotada que permita validarla adecuadamente.
...
La cadena Raw--Tableau está implementada, pero el repositorio no conserva aún
una ejecución final de producción realizada después de incorporar Gold,
DuckDB y el export de Tableau. Por esta razón no se reutilizan como resultado
final los conteos del manifiesto del 2 de septiembre, que corresponde a una
arquitectura anterior. El cierre de esta subsección requiere ejecutar la
cadena sobre el corpus de referencia y conservar los manifiestos de Bronze,
Silver, Gold y Tableau.
...
\toprule
\cabecera{Etapa} & \cabecera{Evidencia pendiente de la ejecución final}\\
\midrule
Bronze & Aceptados, rechazos, tombstones y causas\\
Silver & Filas de \texttt{procurement\_events} y checksum\\
Estado vigente y Gold & Procedimientos, incidencias y oportunidades abiertas\\
DuckDB & Construcción y conteos de las vistas analíticas\\
Tableau & Cinco CSV, filas por fichero y manifest del export\\
\bottomrule
\end{tabularx}
\caption{Evidencia que debe conservar la ejecución E2E final.}
\end{table}

Silver conserva el histórico completo de publicaciones y revisiones. A partir
de él, Gold obtiene una fila por procedimiento para una fecha de corte
concreta. Una licitación se incluye entre las oportunidades abiertas cuando
la evidencia indica que sigue activa y su plazo no ha vencido. Un plazo
terminado o la existencia de una adjudicación, resolución o anulación la
clasifican como cerrada. Los anuncios previos y los casos sin información
suficiente no se muestran como oportunidades abiertas. La fecha de evaluación
se proporciona al comando, por lo que una repetición con los mismos datos y
la misma fecha aplica la misma regla.

\subsection{Explotación: DuckDB y Tableau}

DuckDB expone vistas de solo lectura sobre Gold. El comando
\texttt{export-tableau} escribe cinco CSV con orden determinista y un
manifiesto que registra el conteo de cada fichero.
...
\toprule
\cabecera{Conjunto de datos} & \cabecera{Uso principal} & \cabecera{Granularidad}\\
\midrule
\texttt{open\_opportunities} & Detalle de oportunidades vigentes & Una fila por \texttt{procedure\_id}\\
\texttt{opportunity\_cpv} & Relación entre oportunidades y códigos CPV & Una fila por procedimiento, CPV y posición\\
\texttt{buyer\_summary} & Resumen por comprador & Una fila por identidad analítica del comprador\\
\texttt{cpv\_summary} & Resumen por categoría CPV & Una fila por \texttt{cpv\_code}\\
\texttt{opportunities\_monthly} & Evolución temporal por fuente & Una fila por mes y fuente\\
\bottomrule
\end{tabularx}
\caption{Conjuntos analíticos exportados para Tableau.}
\end{table}

El dashboard permite explorar la evolución temporal, los compradores y las
categorías CPV. Tableau consume estos conjuntos analíticos y no vuelve a
implementar las reglas de estado vigente definidas en Gold. Los conteos de los
cinco CSV y la captura definitiva se incorporarán cuando se conserve el
manifest de la ejecución final.
...
La reproducibilidad se apoya en checksums de procedencia, esquemas versionados,
manifiestos con conteos y exports ordenados. Las medidas que todavía no tienen
un artefacto conservado, como el perfil \texttt{backfill} y la ejecución E2E
final de producción, permanecen identificadas como pendientes.
...
duplicar la lógica de negocio. Falta conservar la ejecución final sobre el
corpus completo y añadir su captura de Tableau; la memoria no presenta esos
resultados como terminados hasta disponer de los artefactos.
...
Completitud & Sin refresco automático de meses cacheados & Estado de ventana y reintentos persistentes\\
Rechazos & El estado global de calidad no integra todavía todos los resultados de ingesta & Unificar la decisión de calidad\\
Ejecución & Batch local reproducible y empaquetado con Docker; sin orquestación & Programación con Airflow\\
Dashboard & Capa de demostración analítica & Personalización opcional por perfil de empresa\\
\bottomrule
\end{tabularx}
