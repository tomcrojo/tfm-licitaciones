...
\subsection{Productos analíticos y Tableau}

DuckDB expone vistas de solo lectura sobre Gold. El comando
\texttt{export-tableau} escribió cinco CSV con orden determinista y un
manifiesto con sus conteos. Para reducir el tiempo de integración, el dashboard
de demostración usa únicamente \path{open_opportunities.csv}; los demás
ficheros se conservan como productos analíticos reproducibles, pero no se
conectan a Tableau.
...
\texttt{opportunities\_monthly.csv} & 0 & Una fila por mes y fuente\\
\bottomrule
\end{tabularx}
\caption{CSV analíticos generados y alcance del dashboard.}
\end{table}

Las 14.763 oportunidades abiertas carecen de \texttt{publication\_date}. Este
campo permanece nullable porque la fuente no siempre lo proporciona y su
ausencia no invalida una oportunidad. Por ello el conjunto mensual, que solo
agrupa registros con fecha de publicación, está vacío. Los otros cuatro CSV
quedan disponibles para análisis posteriores sin formar parte del dashboard.
