...
- `canonical_silver_schema()`: explicit Spark representation of
  `PROCUREMENT_EVENT_SCHEMA`;
- `read_canonical_silver(...)`: typed Parquet read from canonical Silver;
- `schema_from_fields(...)`: mapping from engine-neutral contracts to Spark;
- `assert_contract_schema(...)`: field/type/order validation plus non-null
  checks for required values;
- `write_typed_parquet(...)`: contract validation and Parquet write, with an
  optional logical sort key.

Spark may choose different physical part-file names or byte layouts between
runs. Determinism here means stable schema, stable data semantics and an
explicit logical order when one is requested; consumers must not use part-file
names as identity.
...
1. **Tombstone ordering.** Bronze does not retain Atom `when`; repeated
   ...
   from `ingested_at`.
2. **Source event ordering.** OpenPLACSP revisions with valid
...
2. Freeze `current_state_issues` and source-specific ordering policy.
3. Implement Spark current-state resolution against the contracts above.
