"""Frozen python-row reference baseline for the Silver engine comparison (EXPERIMENT).

This module is a byte-for-byte semantic freeze of the canonical Silver
python-row implementation as it existed at the pre-PR-17 baseline:

    commit e69016f62fb985639e17153e24b3a570f2f39c20 (main, PR #15 merge)
    source: ``src/tfm_licitaciones/silver.py`` (302 lines, sha256
    529f2d1a0a1cc1910baaaa556e16ebab4a571dfa11f3431a9194be166a15aea8)

plus the exact transitive transformation semantics it depended on at that
commit:

    - ``src/tfm_licitaciones/normalize.py``: ``_first_text``,
      ``_parse_date``, ``_normalize_amount_text``, ``_parse_amount``,
      ``_url_from_links``, ``_as_code_list``, ``normalize_ted``,
      ``normalize_boe`` (``normalize_placsp``/``normalize_record`` are
      intentionally NOT copied: the Silver python-row path never calls them;
      the PLACSP Silver path uses ``_first_text`` directly).
    - ``src/tfm_licitaciones/models.py``: ``PROCUREMENT_EVENT_SCHEMA``,
      ``TenderRecord``, ``ProcurementEvent`` (including UTC enforcement in
      ``__post_init__``), ``procurement_events_frame``.
    - ``src/tfm_licitaciones/silver.py``: ``_exact_amount``,
      ``_aware_instant``, ``_single_published_value``, ``_ted_country``,
      ``_ted_event``, ``_placsp_event``, ``_boe_event``,
      ``_placsp_tombstone_event``, ``_record_event``, ``_selection_key``,
      ``_canonical_fields``, ``build_procurement_events``.

Nothing here is "improved" or refactored: identifiers, messages, ordering,
Decimal(20,2) validation, timestamp handling, identity derivation,
provenance selection and collision semantics are preserved verbatim. Only
imports and the module docstring were adapted to make the copy
self-contained (symbol renames: none).

WHY FROZEN: ``bench_engines.py`` labels one engine ``python-row``. Before
this freeze it imported the live production facade
``tfm_licitaciones.silver.build_procurement_events``. After PR #17 that
facade becomes native Polars, so rerunning the experiment would silently
benchmark production Polars under the ``python-row`` label. This module
eliminates that hazard: the ``python-row`` engine now executes this frozen
copy, runnable identically before and after PR #17.

WHAT IS FROZEN vs SHARED:

- FROZEN in this file (no production import; a future change to
  ``src/tfm_licitaciones/silver.py``, ``normalize.py`` or ``models.py``
  cannot change these results): every helper listed above, including TED/BOE
  normalization, amount-text normalization/parsing, Decimal quantization and
  precision checks, timezone-aware instant parsing with UTC normalization,
  ``ProcurementEvent`` validation/UTC coercion, the canonical schema object
  used for frame construction, per-source event mapping, tombstone mapping,
  deterministic provenance selection and canonical-collision detection.
- INTENTIONALLY SHARED (stable experiment harness, not baseline semantics):
  the dataset generator (``tfm_licitaciones.bench_silver``: profiles,
  ``write_bronze_parts``, ``generate_bronze_in_memory``), the Bronze frame
  constructors (``tfm_licitaciones.bronze``) and the parity comparator
  (``tfm_licitaciones.silver_parity``). Those build inputs and check outputs;
  they do not define the ``python-row`` transform. If production ever changes
  the canonical schema, parity will fail loudly (frozen output vs new
  canonical schema) instead of silently moving the historical baseline.

This module has ZERO imports from ``tfm_licitaciones`` (stdlib + polars
only). A static regression test enforces that, plus that no other
experiment module imports the mutable production Silver facade
(``tfm_licitaciones.silver``) as the benchmark oracle.

Historical conclusion (unchanged): Polars won canonical Silver in the
...
IMPLEMENTATION_DETAIL = (
    "frozen historical python-row baseline from e69016f: per-row Python over "
    "a Polars/Parquet frame boundary (iter_rows, json.loads per row, "
    "ProcurementEvent allocation, dict grouping with dedup/collision "
    "comparison, Python sort)"
)
...
# ---------------------------------------------------------------------------
# Frozen data containers (verbatim copies of TenderRecord / ProcurementEvent /
# procurement_events_frame @ e69016f; UTC coercion in __post_init__ is part of
# the frozen timestamp semantics)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Frozen normalization semantics (verbatim copies @ e69016f; only the helpers
# reachable from the Silver python-row path are included)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Frozen Silver python-row transform (verbatim copy of silver.py @ e69016f,
# minus the production imports which are satisfied by the frozen copies above)
# ---------------------------------------------------------------------------
...
def _ted_event(payload: dict[str, Any], row: dict[str, Any]) -> ProcurementEvent:
    ...
    if estimated is None:
        ...
        currency = _first_text(payload.get("currency")) or None
        if currency is None:
            currency = "EUR"
...
def build_procurement_events(records: pl.DataFrame, tombstones: pl.DataFrame) -> pl.DataFrame:
    ...
    Frozen historical baseline (see module docstring): repeated observations
    of the same source event collapse to the row chosen by the deterministic
    minimum provenance tuple. Any other canonical difference under the same
    ``event_id`` fails explicitly instead of picking a latest or earliest
    payload. The result is sorted ascending by ``event_id``.
    """
...
def build_from_parquet(records_glob: str, tombstones_glob: str) -> pl.DataFrame:
    """Read partitioned Bronze Parquet and build Silver with the frozen baseline.

    Experiment-only I/O convenience (not part of the frozen semantics): it
    only reads the same part files the runner passes to the other engines
    and delegates to :func:`build_procurement_events`.
    """
